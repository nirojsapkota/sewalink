...
RSpec.describe Payments::LedgerManager do
  ...
  let(:task) { Task.create!(user: poster, category: category, title: "Cleaning Task", description: "Clean my room", budget_cents: 100000, location: "Kathmandu", status: :assigned, payment_type: :cash) }
  let!(:bid) { Bid.create!(task: task, user: tasker, amount: 1000, status: :accepted, payment_method: :cash) }
